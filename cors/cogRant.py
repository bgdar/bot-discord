...
class CogRant(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        """
        Bot terima setiap message di setiap cannel
        Ini bakal lop terus dalam pengecekannya
        """

        await message.channel.send(
            f"Hey {message.author}, callback message di terima"
        )


# class CogRantStatic(commands.Cog):
#     """Deteksi Kata kata kasar
#     Bgdar : Ini cara lama dan fondasai ide untuk membagun Rant App"""
#
...
#         """kembalikan status data user saat ini :
#             - data user
#             - percentace rant pada suatu user"""
...
#         await ctx.send(f"Kata kasar {kata}")
    ...
    #             responses.append(
    #                 f"- `{word}` → kategori: {name}, level: {level}")
    #
    #         await message.channel.send(
